...
//
// Original: https://github.com/oleganza/CoreBitcoin/blob/4d85239705090079a8664afd28d690ccbeb8f929/CoreBitcoin/BTCMnemonic.m
//
...
@property(nonatomic, readwrite) MnemonicWordListType wordListType;
@property(nonatomic, readwrite) NSData* entropy;
@property(nonatomic, readwrite) NSArray<NSString *>* words;

@end

static inline BOOL MnemonicIsBitSet(uint8_t* buf, int bitIndex) {
...
static inline void MnemonicSetBit(uint8_t* buf, int bitIndex) {
    int value = ((int) buf[bitIndex / 8]) & 0xFF;
    value = value | (1 << (7 - (bitIndex % 8)));
    buf[bitIndex / 8] = (uint8_t) value;
}

static inline void MnemonicIntegerTo11Bits(uint8_t* buf, int bitIndex, int integer) {
    for (int i = 0; i < 11; i++) {
        if ((integer & 0x400) == 0x400) {
            MnemonicSetBit(buf, bitIndex + i);
        }
        integer = integer << 1;
    }
}

static inline NSUInteger MnemonicIntegerFrom11Bits(uint8_t* buf, int bitIndex) {
    NSUInteger value = 0;
    for (int i = 0; i < 11; i++) {
        if (MnemonicIsBitSet(buf, bitIndex + i)) {
            value = (value << 1) | 0x01;
        } else {
            value = (value << 1);
        }
...
- (NSArray*) wordsForEntropy:(NSData*)entropy wordListType:(MnemonicWordListType)listType {
    if (!entropy) return nil;
    ...
    NSData* checksumHash = [self sha256:entropy];
    ...
    for (int i = 0; i < wordsCount; i++) {
        NSUInteger wordIndex = MnemonicIntegerFrom11Bits((uint8_t*)buf.bytes, i * 11);
...
- (NSData*) entropyFromWords:(NSArray*)words wordListType:(MnemonicWordListType)listType {
    if (listType == MnemonicWordListTypeUnknown) return nil;
    if (!words) return nil;
    ...
    for (int i = 0; i < words.count; i++) {
        ...
        MnemonicIntegerTo11Bits((uint8_t*)buf.mutableBytes, i * 11, (int)wordIndex);
    }
...
+ (NSArray*) wordListForType:(MnemonicWordListType)type {
    if (type == MnemonicWordListTypeEnglish) {
