...
+ (instancetype) seedWithEntropy:(NSData *)entropy {
    if (entropy == nil) {
        NSLog(@"entropy should not be nil");
        return nil;
    }
    ...
    Byte *dataBuffer = (Byte *)[entropy bytes];
    for (int i = 0; i < 32; i++) {
        Byte xor = crc ^ dataBuffer[i];
...
+ (instancetype) seedWithWords:(NSArray<NSString *> *)allWords {
    if (allWords == nil) {
        NSLog(@"words should not be nil");
        return nil;
    }
    if (allWords.count != 22) {
    ...
    for (NSString *word in allWords) {
        unsigned int index = (unsigned int )[words indexOfObject:word.lowercaseString];
        if ( index == NSNotFound) {
    ...
    int dataLen = 33;
    NSArray<NSNumber *> *toDigits = [HGCSeed convertRadix:indices fromRadix:(int)words.count toRadix:256 toLength:dataLen];
    Byte crc = toDigits.lastObject.charValue;
    NSMutableData *entropy = [NSMutableData data];
    for (int i = 0; i < toDigits.count-1; i++) {
        unsigned char num = toDigits[i].unsignedCharValue;
        Byte xor = num^crc;
...
+ (instancetype)seedWithBip39Words:(NSArray<NSString *> *)words {
    Mnemonic *mnemonic = [[Mnemonic alloc] initWithWords:words];
    if (mnemonic != nil) {
        return [HGCSeed seedWithEntropy:mnemonic.entropy];
    }
    return nil;
}

+ (Byte)crc8:(NSData *)data length:(NSUInteger)length {
    int crc = 0xFF;
...
- (NSData *)entropy {
    Byte *dataBuffer = (Byte *)[self.scrumbledData bytes];
    Byte crc = dataBuffer[32];
    NSMutableData *mutableData = [[NSMutableData alloc] initWithCapacity:32];
    for (int i = 0; i < 32; i++) {
        Byte xor = crc ^ dataBuffer[i];
        [mutableData appendBytes:&xor length:1];
    }
...
- (NSArray<NSString *> *)toWords {
    ...
    for (NSNumber *index in indexes) {
        [words addObject:allWords[index.unsignedIntValue]];
    }
    return words;
}

- (NSArray<NSString *> *)toBIP39Words {
    Mnemonic *mnemonic = [[Mnemonic alloc] initWithEntropy:[self entropy]];
...
+ (NSArray <NSNumber *> *)convertRadix:(NSArray <NSNumber *> *)number fromRadix:(int)fromRadix toRadix:(int)toRadix toLength:(int)toLength {
    ...
    for (int i = toLength - 1; i >= 0; i--) { // convert BigInteger ==> result
        NSArray<JKBigInteger *> *pair = [num divideAndRemainder:toR];
        num = pair[0];
        [result insertObject:@([pair[1] unsignedIntValue]) atIndex:0];
    }
    return result;
}

+ (NSArray<NSNumber *> *) arrayFromData:(NSData *)data {
    NSMutableArray<NSNumber *> *ary = [NSMutableArray array];
    unsigned char *values = (unsigned char *)[data bytes], cnt = (int)[data length];
    for (int i = 0; i < cnt; ++i) {
        unsigned char v = values[i];
        [ary addObject:[NSNumber numberWithUnsignedChar:v]];
    }
    return ary;
}
...
+(NSArray<NSString *> *)words {return @[@"aback",@"abbey",@"abbot",
